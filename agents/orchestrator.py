...
from models import Club, Player, WhatsAppUser, Match, PointTransaction, Category, WhiteList
...
class Orchestrator:
    ...
    def _extraer_expediente_socio(self, jugador: Player):
        ...
        return {
            ...
            "rango_prestigio": jugador.prestige_rank,
    ...
    def procesar_intencion(self, intencion: dict):
        """
        MOTOR AGÉNTICO V27.0 - EDICIÓN "CERO FRICCIÓN GLOBAL".
        Misión: Gestión autónoma total con persistencia de contexto y fábrica de clubes.
        """
        # --- 1. [OBSERVAR] ---
        ...
        # --- 3. [RAZONAR] ---
        print(f"\033[1;36m[CEREBRO/ORQUESTADOR] -> Analizando acción de {nombre_contexto} | Intento: {tipo}\033[0m")
        ...
        if tipo == "autorizar_socio" and rol in ["SUPER_ADMIN", "ADMIN"]:
            print(f"\033[1;93m[LOOP: PASO 4 - PLANIFICAR 📋] -> Plan: Validar WhiteList -> Registrar nuevo acceso -> Verificar.\033[0m")
            ...
            try:
                ...
                if check:
                    print(f"\033[1;32m[LOOP: PASO 6 - VERIFICAR ✅] -> Acceso confirmado físicamente en DB.\033[0m")
        ...
        if tipo == "configurar_categorias" and rol in ["SUPER_ADMIN", "ADMIN"]:
            print(f"\033[1;93m[LOOP: PASO 4 - PLANIFICAR 📋] -> Plan: Purgar categorías actuales -> Insertar menú nuevo -> Verificar integridad.\033[0m")
        ...
        # C. CREAR NUEVO CLUB (PILAR 10: LA FÁBRICA DE CLUBES)
        if tipo == "crear_nuevo_club" and rol == "SUPER_ADMIN":
            print(f"\033[1;93m[LOOP: PASO 4 - PLANIFICAR 📋] -> Plan: Crear instancia de Club -> Autorizar Admin VIP.\033[0m")
            nombre_club = datos_nuevos.get("nombre_club")
            tel_admin = datos_nuevos.get("telefono_admin")
            
            if not nombre_club or not tel_admin:
                return {"status": "info", "perfil_socio": expediente, "orden_ia": "ORDEN: Faltan datos para fundar el club. Solicítelos."}
            
            try:
                # --- 5. [EJECUTAR] ---
                existe_club = self.db.query(Club).filter_by(name=nombre_club).first()
                if existe_club: return {"status": "error", "orden_ia": f"ORDEN: Informe que el club '{nombre_club}' ya existe."}
                
                nuevo_c = Club(name=nombre_club, admin_phone=tel_admin)
                self.db.add(nuevo_c); self.db.flush() 

                nueva_a = WhiteList(phone_number=tel_admin, full_name=f"Admin {nombre_club}", club_id=nuevo_c.id)
                self.db.add(nueva_a); self.db.commit()

                # --- 6. [VERIFICAR] ---
                check = self.db.query(Club).filter_by(id=nuevo_c.id).first()
                if check:
                    print(f"\033[1;32m[LOOP: PASO 6 - VERIFICAR ✅] -> ¡Imperio expandido! Club '{nombre_club}' operativo.\033[0m")
                    return {
                        "status": "config_success", 
                        "nuevo_club_id": nuevo_c.id, 
                        "perfil_socio": expediente, 
                        "orden_ia": f"ORDEN: Confirme que el club '{nombre_club}' ha sido fundado y el administrador habilitado."
                    }
            except Exception as e:
                self.db.rollback(); return {"status": "error", "orden_ia": f"ORDEN: Error en la expansión: {str(e)}"}
        ...
        if tipo == "consultar_analitica" and jugador:
            ...
            # --- 6. [VERIFICAR] ---
            if "error" not in reporte_analista:
        ...
        if tipo in ["crear_reto", "reproponer_reto", "reproponer_fecha"]:
            ...
            for k in ["rival", "dia", "hora", "fecha_iso", "categoria"]:
                ...
                if val and str(val).lower() not in ["null", "none", "unknown"]:
                    # 🛡️ FILTRO ANTI-AMNESIA: No permitimos que Daniel se rete a sí mismo por error de la IA
            ...
            if res_reto["status"] == "challenge_proposed":
                # --- 6. [VERIFICAR] ---
                check_match = self.db.query(Match).filter_by(id=res_reto.get("match_id")).first()
                if check_match:
                    print(f"\033[1;32m[LOOP: PASO 6 - VERIFICAR ✅] -> Duelo ID {check_match.id} confirmed en DB.\033[0m")
        ...
        # ✅ MEJORA CRÍTICA: ACEPTAR RETO BLINDADO
        if tipo == "aceptar_reto":
            # Buscamos cualquier reto donde el usuario sea P1 o P2 y esté en estado 'proposed'
            match_p = self.db.query(Match).filter(
            ...
            if match_p:
                print(f"\033[1;32m🤝 [HANDSHAKE] -> {jugador.name} está aceptando el Match ID {match_p.id}.\033[0m")
                match_p.status = "scheduled"; self.db.commit()
                
                # Identificamos a quién notificar (al otro jugador)
                otro_tel = match_p.player_2.owner.phone_number if match_p.player_1_id == jugador.id else match_p.player_1.owner.phone_number
                
                return {
                    "status": "challenge_confirmed", 
                    "perfil_socio": expediente, 
                    "notificar_a": otro_tel, 
                    "mensaje_proactivo": f"✅ ¡Duelo confirmado en el Muro de la Fama! {jugador.name} aceptó.", 
                    "orden_ia": "ORDEN: Celebra la confirmación y la valentía."
                }
            
            print(f"\033[1;31m⚠️ [AVISO] -> {jugador.name} dijo 'acepto' pero no tiene retos propuestos.\033[0m")
            return {"status": "chat_asistente", "perfil_socio": expediente, "orden_ia": "ORDEN: Informe con distinción que no tiene retos pendientes por aceptar."}
